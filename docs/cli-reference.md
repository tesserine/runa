...
live in `.runa/project.toml`, or the path named by `project_config` in the
local file.

```toml
# .runa/config.toml
methodology_path = "../groundwork/manifest.toml"
...
`[[forges.instances]]` declares a forge type and service hosts. Repositories
and trackers then name an instance and supply the resource coordinates runa
uses for project config, context/env payloads, scoped work-unit identity, forge
mechanics, and transcript deployment identity. Runa injects the resolved
`RUNA_FORGE_ADDRESSES` payload into launched agent and MCP environments.
...
Live `runa step`, `runa run`, and `runa go` require a `[launch].command` entry
in `.runa/project.toml`:
...
number, `#<N>`, or `<tracker-selector>#<N>`. Bare references are accepted only
when the project has exactly one configured tracker; qualified references
select a configured tracker from `.runa/project.toml`. When the referenced
...
- `RUNA_FORGE_ADDRESSES` — Canonical forge-address payload delivered by runa
  to launched agents and MCP servers. It is resolved from `.runa/project.toml`;
  callers do not construct or override it directly.
