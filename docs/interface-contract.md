...
- Freshness suppression uses successful execution records when available. After a protocol finishes with passing postconditions, runa records the valid freshness-relevant input set it processed for that `(protocol, work_unit)` pair. Later evaluations suppress reruns only when the current valid input set matches that recorded set. If no execution record exists, runa falls back to output artifact timestamps. Protocols with no `produces` types are never suppressed by freshness.
...
For `on_change` protocols, timestamps still decide whether the trigger fires. A successful execution then records which valid inputs were actually processed. This lets runa suppress reruns when the input set is unchanged even if invalid siblings appear or a successful rerun rewrites identical output content. When no execution record exists yet, the timestamp fallback still applies.
