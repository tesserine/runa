...
Optional fields declare the protocol's relationship to artifacts:
...
- **required_output_choices** — nested tables that name mutually exclusive required output groups. After execution, exactly one member artifact type in each group must exist and validate.
...
Required output choices use TOML array-of-table syntax:
...
Unscoped protocols must not declare outputs whose schemas require `work_unit`. If an artifact schema named in `produces`, `may_produce`, or a required output choice member lists `work_unit` in its top-level `required` array, the protocol must declare `scoped = true`. Otherwise the manifest is invalid and `runa init` rejects it.

The example above uses `requires` and `produces`. See the [interface contract](interface-contract.md) for the full semantics of `accepts`, `may_produce`, and required output choices.
