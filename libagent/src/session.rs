//! Shared session-surface evaluation helpers.
//!
//! The CLI commands and `runa-mcp` session surface both use these helpers so
//! readiness and context planning stay under one runtime authority.
...
use crate::{
    ArtifactFailure, ArtifactStore, EvaluationScope, EvaluationTopology, ExecutionRecord,
    LoadedProject, ProtocolDeclaration, ScanResult, ValidationStatus, WaitingReason,
};
...
pub struct EvaluatedProtocols {
    pub topology: EvaluationTopology,
...
pub struct SessionState {
    ...
    pub planned_entries: Vec<SessionPlanEntry>,
}
...
pub fn evaluate_protocols(
    ...
    let protocol_order: Vec<&str> = topology.status_order.iter().map(String::as_str).collect();
    ...
    for candidate in classified {
        let Some(protocol) = protocol_map.get(candidate.protocol_name.as_str()) else {
            continue;
        };
        ...
        let entry = match candidate.status {
            crate::CandidateStatus::Ready if in_cycle => ProtocolEntry {
                ...
                waiting_reason: Some(WaitingReason::TriggerUnsatisfied),
            },
