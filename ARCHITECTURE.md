...
Runs the same implicit scan and shared candidate classification used by `runa state`, then narrows the plan to the single next concrete `READY` `(protocol, work_unit)` pair in non-cyclic graph order. Candidate discovery, trigger evaluation, and freshness suppression still use the work-unit-scoped selection logic in `selection.rs`; the only change is that `step` now previews and executes one candidate instead of a whole cascade.
...
Runs the same implicit scan and shared candidate classification as `step`, but keeps selecting work until quiescence instead of stopping after one execution. `run --dry-run` now projects that cascade from graph state only: declared `produces` outputs, dependency edges, trigger declarations, and the current evaluated work-unit topology. The projection tracks assumed-success output availability and freshness in memory without synthesizing artifact payloads, forking the store, or recording assumed-valid shadow artifacts. Initially ready entries retain MCP config and full context only on their first emission; reopened reruns later in the projected cascade are marked projected and omit filesystem-backed context details. Projected work units are inferred from manifest relationships plus current artifact-state work units, including newly projected produced work.
