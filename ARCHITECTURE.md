...
Without `--dry-run`, `run` exits `0` when the topology is fully satisfied, `2` when any protocol failed or violated postconditions during the invocation, `3` when work remains blocked or waiting on external input after all runnable work is exhausted, and `130` when `Ctrl-C` is received between execution cycles. Interruption is boundary-scoped on the first `Ctrl-C`: the active agent run is isolated from the terminal SIGINT on Unix, the current execution still completes its scan and postcondition reconciliation, and the command stops before launching the next candidate. A second `Ctrl-C` force-exits `runa` immediately with status `130`; because the live agent is already isolated in its own process group on Unix, that forced exit can leave the child process running after `runa` terminates. Failed candidates are still skipped for the rest of that invocation, and successful executions, postcondition-failing reconciliations, and agent-failing reconciliations can still reopen exhausted candidates when later relevant inputs change.
