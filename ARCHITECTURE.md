...
Without `--work-unit`, `state` evaluates only `scoped = false` protocols and each protocol appears at most once with no `work_unit`. With `--work-unit <ID>`, `state` evaluates only `scoped = true` protocols for that exact delegated work unit. When valid delivered `work-unit` artifacts exist, `<ID>` must exactly match one delivered work-unit instance id; non-exact values reject before readiness evaluation. No readiness path enumerates sibling work units from artifact state.
