...
- **`runa-forge-capability`** — Vendored commons forge capability v1.0.0 contract (`tesserine/commons@f58ee912b226a8db31902630205bc5ee50b5ee34`), opaque handle type, operation list, and schema validation helpers.
- **`runa-connector-github`** and **`runa-connector-sourcehut`** — Provider connectors for the eight forge operations. Provider coordinate grammar and handle parsing live here, not in libagent or the MCP surface.
- **`runa-connector-registry`** — Composition layer that loads the configured connector and aliases, then gives `runa-mcp` provider-agnostic tool bindings.
...
Session mode opens a scoped libagent session and serves the unified driver,
output-tool, and configured connector-tool surface.

### `handler.rs`

`ServerHandler` implementation. `RunaHandler` derives one MCP tool per output artifact type (`produces` + required output choice members + viable `may_produce`), with tool input schemas derived from artifact type JSON Schemas (with `work_unit` stripped). It also asks the connector registry for configured forge tools and appends them with commons-derived input/output schemas. `validate_protocol_scope` rejects scoped protocols without `--work-unit` and unscoped protocols with one. `validate_output_types` remains a defense-in-depth guard for required output schemas unsupported by MCP tool generation, while sharing the same unscoped-output `work_unit` predicate used by manifest parsing. In fixed-protocol mode, `call_tool` validates artifacts before writing, then writes to the workspace and records in the store. In session mode, the handler also exposes `readiness`, `next-protocol-context`, and `advance`; output tools always derive from the current step, and any declared output type for that step that collides with a reserved driver name is refused before the step is entered. Connector calls validate input and output against the forge capability schemas and return MCP structured content.
