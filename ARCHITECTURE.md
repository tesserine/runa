...
9. **MCP runtime loop.** `runa-mcp` parses `--protocol` and optional `--work-unit`, loads the project, scans the artifact workspace into the store, resolves the named protocol from the manifest, validates declared scope and scoped work-unit identity against the refreshed state, validates that its outputs can be served as MCP tools, and serves an MCP session via stdio. When transcript capture is enabled, tool calls and tool results are appended to the same JSONL stream as CLI execution events.
...
Runtime loop: parses `--protocol` and optional `--work-unit`, loads the project, scans the artifact workspace into the store, resolves the named protocol from the manifest, validates declared scope and scoped work-unit identity against refreshed state, validates its output types, builds the MCP handler, and serves via stdio transport.
...
Without `--work-unit`, `state` evaluates only `scoped = false` protocols and each protocol appears at most once with no `work_unit`. With `--work-unit <ID>`, `state` evaluates only `scoped = true` protocols for that exact delegated work unit. When any delivered `work-unit` artifacts are recorded, `<ID>` must exactly match one delivered work-unit instance id; non-exact values reject before readiness evaluation. Handle-number agreement and duplicate full-ticket-identity checks apply only to valid, parseable work-unit artifacts. No readiness path enumerates sibling work units from artifact state.
