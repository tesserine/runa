...
Forge work-unit acquisition used by `runa take <id>`. Resolves GitHub or
SourceHut identity from loaded config, reads credentials from env-only token
names, fetches tracker JSON through an injectable fetcher, extracts list-item
acceptance criteria from recognized headings, validates the resulting
`work-unit` artifact, writes `.runa/workspace/work-unit/<id>.json`, and records
it in the store.
...
agreement from config-resolved `GROUNDWORK_FORGE_*` atoms. Tracker-number
agreement accepts both `work-unit-<number>` style ids and bare numeric ids.
