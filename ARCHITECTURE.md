...
8. **CLI step execution.** `runa step` reuses the same execution-plan construction as dry-run, then, when `[agent].command` is configured, invokes that argv command once per non-cyclic ready plan entry in order. Each entry carries both the rendered `ContextInjection` prompt and an MCP server configuration describing how the agent runtime should spawn `runa-mcp` for that candidate. Live execution exports that config as `RUNA_MCP_CONFIG`, writes the prompt on stdin, runs in the project working directory, and requires exit status `0` for step to continue. This issue stops at command execution: no post-execution scan, postcondition enforcement, or cascading re-selection happens yet.
