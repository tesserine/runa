...
Runs the same implicit scan and shared candidate classification as `step`, but keeps selecting work until quiescence instead of stopping after one execution. `run --dry-run` projects the optimistic cascade by simulating successful required outputs from each planned execution; projected outputs are inserted into the shadow store as assumed-valid artifacts without schema instance validation so downstream readiness follows declared topology rather than the synthetic value generator's coverage of JSON Schema. Initially ready entries retain MCP config and full context only on their first emission; reopened reruns later in the projected cascade are marked projected and omit filesystem-backed context details. The projection also merges composed output schemas before synthesis: `allOf` branches are combined, and sibling constraints from the parent schema are merged into the selected first `oneOf`/`anyOf` branch. When a candidate has a `work_unit`, the projection injects it into the synthesized object before recording the shadow artifact, so projected readiness stays work-unit scoped even when the schema omits `properties.work_unit`.

Without `--dry-run`, `run` exits `0` when the topology is fully satisfied, `2` when any protocol failed or violated postconditions during the invocation, and `3` when work remains blocked or waiting on external input after all runnable work is exhausted. It skips failed candidates for the rest of that invocation while still reconciling any artifacts emitted before failure, allowing successful executions, postcondition-failing reconciliations, and agent-failing reconciliations to reopen exhausted candidates when later relevant inputs change.
