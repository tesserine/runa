...
8. **CLI step execution.** `runa step` reuses the same execution-plan construction as dry-run, then, when `[agent].command` is configured, invokes that argv command once per non-cyclic ready plan entry in order. Each invocation renders the entry's `ContextInjection` into a natural-language prompt, includes the selected `work_unit` in the heading when present, writes that prompt on stdin, runs in the project working directory, and must exit `0` for step to continue. This issue stops at command execution: no post-execution scan, postcondition enforcement, or cascading re-selection happens yet.
