...
`run --dry-run` projects the full optimistic cascade from declared `produces` outputs only; `may_produce` outputs remain optional and do not advance the projection unless they already exist on disk. Initially ready entries include the same MCP config and context shape used by `step --dry-run` only on their first concrete emission, and those contexts stay tied to artifacts that actually exist on disk; any later reopened rerun in the projection is marked as projected because the real-store context no longer matches the simulated invocation. Projected outputs are recorded in the dry-run shadow store as structurally plausible assumed-success artifacts, so downstream readiness is not gated on the synthetic value satisfying every JSON Schema constraint. For produced composed schemas, the projection merges `allOf` branches before synthesis and also merges parent sibling constraints with the selected first `oneOf`/`anyOf` branch so projected values keep required top-level fields such as `work_unit`. Its exit status still reflects the current evaluated topology after the initial scan rather than forcing success because a projection was printed.
