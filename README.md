...
`run` is the cascade command. Live execution walks the same non-cyclic READY frontier as `step`, but continues until quiescence instead of stopping after one protocol. Previously exhausted work reopens only when a later successful execution or postcondition-failing reconciliation changes inputs that are relevant to that candidate. Agent failures and postcondition failures are tolerated for the remainder of the invocation: the failed candidate is skipped, other READY work continues, and the command exits `2` after quiescence if any protocol failed. If no READY work exists and some work remains blocked, waiting on external input, or trapped in a hard dependency cycle, `run` exits `3`. A fully satisfied topology exits `0`.

`run --dry-run` projects the full optimistic cascade from declared `produces` outputs only; `may_produce` outputs remain optional and do not advance the projection unless they already exist on disk. Initially ready entries include the same MCP config and context shape used by `step --dry-run` only on their first concrete emission, and those contexts stay tied to artifacts that actually exist on disk; any later reopened rerun in the projection is marked as projected because the real-store context no longer matches the simulated invocation. Projected outputs are recorded in the dry-run shadow store as structurally plausible assumed-success artifacts, so downstream readiness is not gated on the synthetic value satisfying every JSON Schema constraint. For produced composed schemas, the projection merges `allOf` branches before synthesis and also merges parent sibling constraints with the selected first `oneOf`/`anyOf` branch. When the candidate is scoped, the projection injects `work_unit` into the synthesized object before recording the shadow artifact, preserving downstream work-unit scoping even when the schema does not declare `properties.work_unit`. Its exit status still reflects the current evaluated topology after the initial scan rather than forcing success because a projection was printed.
