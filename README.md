...
Scope is methodology-owned metadata, not something runa infers from schemas or artifact contents. Unscoped commands evaluate only unscoped protocols. `runa state --work-unit <ID>`, `runa step --work-unit <ID>`, and `runa run --work-unit <ID>` evaluate only `scoped = true` protocols for that delegated work unit. When any `work-unit` artifacts are recorded, `<ID>` must exactly match one delivered work-unit instance id; non-exact values are rejected. Tracker-backed handle agreement is checked only for valid work-unit artifacts.
