...
Without `--dry-run`, `step` requires `[agent].command`. If no READY work exists, it prints `No READY protocols.` and exits without requiring `runa-mcp`. Otherwise it resolves `runa-mcp` and executes ready work one candidate at a time in topological order. For each invocation, runa exports the candidate's MCP config in `RUNA_MCP_CONFIG`, renders the candidate's context into a natural-language prompt, including the scoped `work_unit` in the heading when present, and writes that prompt to the child process on stdin before waiting for exit status `0`. After each successful exit, runa re-scans the workspace, enforces postconditions for the completed `(protocol, work_unit)`, re-evaluates readiness from the refreshed store, and selectively reopens exhausted READY work only when that scan recorded artifact-state transitions (`new`, `modified`, `removed`, or `revalidated`) in artifact types that overlap the candidate's required or trigger-referenced inputs. Persistent scan warnings alone do not restart exhausted READY work. A non-zero exit still stops execution immediately and skips the post-execution reconciliation cycle.
