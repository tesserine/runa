...
`run --dry-run` projects the full optimistic cascade from declared `produces` outputs only; `may_produce` outputs remain optional and do not advance the projection unless they already exist on disk. Current concrete entries include the same MCP config and context shape used by `step --dry-run`, and those contexts stay tied to artifacts that actually exist on disk; downstream projected entries are marked as projected because their artifacts do not yet exist on disk. Projected outputs are recorded in the dry-run shadow store as structurally plausible assumed-success artifacts, so downstream readiness is not gated on the synthetic value satisfying every JSON Schema constraint. For produced `allOf` schemas, the projection still merges the composed branches before synthesis so projected values stay type-plausible. Its exit status still reflects the current evaluated topology after the initial scan rather than forcing success because a projection was printed.
