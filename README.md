...
Scope is methodology-owned metadata, not something runa infers from schemas or artifact contents. Unscoped commands evaluate only unscoped protocols. `runa state --work-unit <ID>`, `runa step --work-unit <ID>`, and `runa run --work-unit <ID>` evaluate only `scoped = true` protocols for that delegated work unit. When valid `work-unit` artifacts exist, `<ID>` must exactly match one delivered work-unit instance id; non-exact values are rejected.
